...
def plot_results(opt_output, plotOption='w', writePath='./output/results.png'):
    ...
    fig.suptitle('Original Load = ' + "{:.2f}".format(original_load_kwh) +
                 ' kWh' +
                 ' | Optimized Load = ' +
                 "{:.2f}".format(optimized_load_kwh) +
                 ' kWh', fontsize=12)
    ...
    legend_elements = [Line2D([0], [0], color='royalblue', lw=2, label='EWH Water Temperature'),
                       Line2D([0], [0], color='lightgreen', lw=2, label='User-Defined Hot Water Confort Temperature',
                              linestyle='dashed'),
